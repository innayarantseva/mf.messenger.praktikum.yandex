...
const CHAT_OPTIONS_MENU_OPENED_CLASS_SELECTOR = 'conversation__options-menu_opened_true';
...
const CHAT_ATTACHEMENT_MENU_OPENED_CLASS_SELECTOR = 'conversation__attachment-menu_opened_true';
...
const renderByContext = (context) => {
    ...
    // handle chat click
    const chatsList = document.getElementsByClassName(CHATS_LIST_CLASS_SELECTOR)[0];

    chatsList.addEventListener('click', (event) => {
        const closestChatItemParent = event.target.closest(CHAT_SELECTOR);

        if (closestChatItemParent) {
            const chatTitle = closestChatItemParent.dataset.chatTitle;
            const newContext = {
                ...context,
                title: chatTitle,
    ...
    const chatOptions = document.getElementsByClassName(CHAT_OPTIONS_CLASS_SELECTOR)[0];
    if (chatOptions) {
        chatOptions.addEventListener('click', () => {
            const chatOptionsMenu = chatOptions.getElementsByClassName(CHAT_OPTIONS_MENU_CLASS_SELECTOR)[0];
            chatOptionsMenu.classList.toggle(CHAT_OPTIONS_MENU_OPENED_CLASS_SELECTOR);
        })
    }

    const chatAttachment = document.getElementsByClassName(CHAT_ATTACHEMENT_CLASS_SELECTOR)[0];
    if (chatAttachment) {
        chatAttachment.addEventListener('click', () => {
            const chatAttachmentMenu = chatAttachment.getElementsByClassName(CHAT_ATTACHEMENT_MENU_CLASS_SELECTOR)[0];
            chatAttachmentMenu.classList.toggle(CHAT_ATTACHEMENT_MENU_OPENED_CLASS_SELECTOR);
        })
    }
